    ...
    for model_config in configs:
        if model_config.type == "damage":
            ...
            if model_config.weight == 2:
                preds.append(damage)
    return np.average(preds, axis=0)  # 返回(h, w, 5)格式
...
def post_process(loc, damage, out_loc, out_damage):
    ...
    first = np.zeros((h, w, 1))  # 使用实际大小而不是硬编码1024
...
def main():
    ...
    damage = predict_damage_ensemble(args.pre, args.post)  # (h, w, 5)
    
    # 确保数据类型正确
    damage = (damage * 255).astype(np.uint8)  # 转换为uint8类型
    damage_for_save = damage[:, :, 1:].copy()  # 只取后4个通道
