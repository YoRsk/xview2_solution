...
def predict_localization(image, config: ModelConfig):
    ...
    checkpoint = torch.load(checkpoint_path, map_location="cuda")  # 直接加载到GPU
    model.load_state_dict({k.replace("module.", ""): v for k, v in checkpoint['state_dict'].items()})
    model.eval()
    model = model.cuda()  # 直接使用GPU
    ...
    with torch.no_grad():
        for y in range(0, h, stride):
            for x in range(0, w, stride):
                ...
                window = img_to_tensor(window, normalize)  # 保持tensor格式
                
                if "dpn" in config.weight_path:
                    window = np.pad(window.numpy(), 
                                  [(0, 0), (16, 16), (16, 16)], 
                                  mode='reflect')
                ...
                aug_predictions = []
                for i, aug_window in enumerate(aug_windows):
                    try:
                        # 送入GPU进行预测
                        logits = model(aug_window.unsqueeze(0).cuda().float())
...
def predict_damage(image, config: ModelConfig):
    ...
    output = np.zeros((h, w, 5), dtype=np.float32)
    counts = np.zeros((h, w), dtype=np.float32)
    ...
    with torch.no_grad():
        for y in range(0, h, stride):
            for x in range(0, w, stride):
                ...
                window = img_to_tensor(window, conf["input"]["normalize"])
                ...
                for i, aug_window in enumerate(aug_windows):
                    try:
                        logits = model(aug_window.unsqueeze(0).cuda().float())
                        pred = torch.softmax(logits, dim=1).cpu().numpy()[0]
    ...
    valid_mask = counts > 0
    output[valid_mask] /= counts[valid_mask, np.newaxis]
    
    return output
def predict_localization_ensemble(pre_path):
    """Ensemble prediction for localization"""
...
def predict_damage_ensemble(pre_path, post_path):
    """Ensemble prediction for damage assessment"""
...
def post_process(loc, damage, out_loc, out_damage):
    ...
    loc = ((localization > 0.25) | (argmax > 0)).astype(np.uint8)
...
def main():
    ...
    image_pre = cv2.imread(args.pre, cv2.IMREAD_COLOR)[:, :, ::-1]
    image_post = cv2.imread(args.post, cv2.IMREAD_COLOR)[:, :, ::-1]
